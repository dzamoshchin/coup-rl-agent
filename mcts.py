...
class MCTSPlayer(Player):
    def __init__(self, Q: Dict[int, Dict[int, int]], N: Dict[int, Dict[int, int]], c: int, depth: int,
                 num_simulations: int):
        super().__init__()
    ...
    def simulation(self, legal_moves: List[Tuple[Move, int]], history: Tuple[Tuple[int, int]], depth: int,
                   gamma: int = 1) -> int:
        if (depth <= 0):
