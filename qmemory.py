...
class QMemoryPlayer(Player):
    def __init__(self,
        ...
        self.action_histories = [[0,0,0] for _ in range(num_players)]
    ...
    def get_observation(self, obs: Observation) -> None:
        ...
        if obs.obs_type == ObservationType.MOVE:
            if obs.obs == Move.SWAP_CARDS:
                self.action_histories[(obs.player - self.idx)%self.num_players] = [0,0,0]
            if obs.obs == Move.ASSASSINATE:
                self.action_histories[(obs.player - self.idx)%self.num_players][0] = 1
            if obs.obs == Move.STEAL:
                self.action_histories[(obs.player - self.idx)%self.num_players][1] = 1
            if obs.obs == Move.TAX:
                self.action_histories[(obs.player - self.idx)%self.num_players][2] = 1
        ...
        self.cur_game_state = start_on_idx([coin_bucket(c) for c in obs.visible_state.coin_list]) + \
                              ...
                              [tuple(i) for i in self.action_histories[1:]] + \
                              sorted(self.roles)
